...
class ProMap extends Map {
    ...
    public function __construct( Core $core ) {
        ...
        add_filter( 'igm_prepare_meta', [$this, 'prepare_pro_meta'], 10 );
        add_filter( 'igm_model', [$this, 'check_public'], 10 );
        add_filter( 'igm_map_before', [$this, 'prepare_content_wrapper'], 10, 2 );
        add_filter( 'igm_map_after', [$this, 'prepare_content_wrapper'], 10, 2 );
    ...
    public function set_amcharts_filters() {
        if ( isset( $this->options['amcharts_url'] ) && trim( $this->options['amcharts_url'] ) !== '' ) {
            add_filter( 'igm_amcharts_version', [$this, 'amcharts_null_version'] );
            add_filter( 'igm_amcharts_lib_url', [$this, 'amcharts_lib_url'] );
            add_filter( 'igm_amcharts_geodata_url', [$this, 'amcharts_geodata_url'] );
        }
    }
    ...
    public function check_public( $model ) {
        ...
        add_filter( 'the_content', [ $this, 'single_page_map' ] );

        // filter to intercept image url
        add_filter( 'template_redirect', [ $this, 'single_page_image_url' ] ) ;

        return $model;
    }

    public function single_page_image_url() {
        if ( ! is_singular( 'igmap' ) ) {
            return;
        }

        // if the ?image parameter is set in the URL, display the map as an image
        if ( isset( $_GET['image'] ) ) {
            global $post;
            $image_meta  = get_post_meta( $post->ID, 'map_image', true );
            $base64Image = ! empty( $image_meta['mapImage'] ) ? $image_meta['mapImage'] : '';

            if ( empty( $base64Image ) ) {
                return;
            }

            // Split the data URL to extract the format and data
            ...
            if ( strpos( $format, 'jpeg' ) !== false || strpos( $format, 'jpg' ) !== false ) {
                $contentType = 'image/jpeg';
            } elseif ( strpos( $format, 'png' ) !== false ) {
                $contentType = 'image/png';
            } elseif ( strpos( $format, 'gif' ) !== false ) {
    ...
    public function prepare_pro_meta( $meta ) {
        ...
        if ( $custom_icon_markers_source && isset( $custom_icon_markers_source['enabled'] ) && '1' === $custom_icon_markers_source['enabled'] ) {
            ...
            switch ( $custom_icon_markers_source['type'] ) {
                ...
                case 'json_data':
                    ...
                    break;
                case 'google_spreadsheet':
                    $extra_markers       = $this->add_google_spreadsheet_data( 'iconMarkers', $custom_icon_markers_source, $meta['id'] );
                    $meta['iconMarkers'] = array_merge( $meta['iconMarkers'], $extra_markers );
                    break;
            }

            // check if we need to process the action content template
            if ( isset( $custom_icon_markers_source['action_content_template'] ) && ( '' !== $custom_icon_markers_source['action_content_template'] && '{content}' !== $custom_icon_markers_source['action_content_template'] ) ) {
                $meta['iconMarkers'] = $this->process_action_content_template( $meta['iconMarkers'], $custom_icon_markers_source['action_content_template'], 'iconMarkers' );
            }
        }

        // prepare image markers source
        ...
        if ( $custom_image_markers_source && isset( $custom_image_markers_source['enabled'] ) && '1' === $custom_image_markers_source['enabled'] ) {
            ...
            // check if we need to process the action content template
            if ( isset( $custom_image_markers_source['action_content_template'] ) && ( '' !== $custom_image_markers_source['action_content_template'] && '{content}' !== $custom_image_markers_source['action_content_template'] ) ) {
                $meta['imageMarkers'] = $this->process_action_content_template( $meta['imageMarkers'], $custom_image_markers_source['action_content_template'], 'imageMarkers' );
            }
        }

        // lines
        // get coordinates from existing markers, if selected
        if ( isset( $meta['lines'] ) && is_array( $meta['lines'] ) && !empty( $meta['lines'] ) ) {
            foreach ( $meta['lines'] as $lineCollectionIndex => $lineCollection ) {
                foreach ( $lineCollection['multiGeoLine'] as $lineIndex => $line ) {
                    if ( isset( $line['pointCoordinates'] ) && $line['pointCoordinates'] !== 'custom' ) {
                        $meta['lines'][$lineCollectionIndex]['multiGeoLine'][$lineIndex]['coordinates'] = $this->get_marker_coordinates_for_lines( $line['pointCoordinates'], $meta );
                    }
                }
            }
        }
        ...
        if ( ! isset( $this->options['dictionary'] ) || ( isset( $this->options['dictionary'] ) && $this->options['dictionary'] ) ) {
            // need to improve this, since the dictionary already runs before - maybe disable the default one if Pro is enabled?
            // but it needs to run late, to catch all other data sources
    ...
    /**
     * Get marker coordinates based on value which contains type of series and marker title
     * should return coordinates to be used in lines
     *
     * @param [string] $value
     * @param [array] $meta
     * @return array
     */
    public function get_marker_coordinates_for_lines( $value, $meta ) {
        $series = [
            'roundMarkers',
            'imageMarkers',
            'iconMarkers',
        ...
        foreach ( $series as $serie ) {
            if ( strpos( $value, $serie ) === 0 ) {
                $coordinates = [
                    'latitude'  => 0,
                    'longitude' => 10
                ];

                $realValue = str_replace( $serie, '', $value );

                if ( $meta[$serie] && is_array( $meta[$serie] ) ) {
                    $match = array_filter( $meta[$serie], function ( $marker ) use ( $realValue ) {
                        return ( $marker['id'] === $realValue );
                    } );

                    if ( ! empty( $match ) ) {
                        foreach ( $match as $matchedMarker ) {
                            $coordinates = $matchedMarker['coordinates'];
                        }
                    }
                }
            }
     ...
     */
    private function process_action_content_template( $data, $template, $type ) {
        // check if placeholders exist and setup content.
        ...
        if ( ! empty( $placeholders ) ) {
            foreach ( $data as $key => $value ) {
                ...
                // We might not need to replace again on ProActions.php
                // we need to replace now, because of the entries that are merged, they need the content to be processed before the merge
                $data[$key]['content'] = $this->replace_placeholders( $data[$key]['content'], $data[$key] );
            }
        }
    ...
    private function render_html( $string, $mode = false ) {
        if ( $mode !== true ) {
            return nl2br( $string );
        }
        return $string;
    ...
    private function prepare_shortcode_content( $meta ) {
        ...
        if ( ! empty( $meta['tooltip']['renderHTML'] ) ) {
            $render_tooltip_mode = false;
            if ( $meta['tooltip']['renderHTML'] === 'html' ) {
                $render_tooltip_mode = true;
            }

            if ( $meta['tooltip']['renderHTML'] === 'asSettings' ) {
                $render_tooltip_mode = false;
        ...
        $meta['regionsTooltipTemplate']      = isset( $meta['regionsTooltipTemplate'] ) ? $this->render_html( $meta['regionsTooltipTemplate'], $render_tooltip_mode ) : '';
        $meta['roundMarkersTooltipTemplate'] = isset( $meta['roundMarkersTooltipTemplate'] ) ? $this->render_html( $meta['roundMarkersTooltipTemplate'], $render_tooltip_mode ) : '';
        $meta['imageMarkersTooltipTemplate'] = isset( $meta['imageMarkersTooltipTemplate'] ) ? $this->render_html( $meta['imageMarkersTooltipTemplate'], $render_tooltip_mode ) : '';
        $meta['iconMarkersTooltipTemplate']  = isset( $meta['iconMarkersTooltipTemplate'] ) ? $this->render_html( $meta['iconMarkersTooltipTemplate'], $render_tooltip_mode ) : '';
        $meta['labelsTooltipTemplate']       = isset( $meta['labelsTooltipTemplate'] ) ? $this->render_html( $meta['labelsTooltipTemplate'], $render_tooltip_mode ) : '';
            ...
            if ( !empty( $meta[$data_container] ) ) {
                foreach ( $meta[$data_container] as $k => &$entry ) {
                    if ( isset( $entry['tooltipContent'] ) && $entry['tooltipContent'] !== '' ) {
                        $entry['tooltipContent'] = $this->render_html( $entry['tooltipContent'], $render_tooltip_mode );
    ...
    private function add_custom_js( $prefix, $id, $code ) {
        // we might need to change this, we should double check sanitization
    ...
    private function add_json_object_data( $type, $json_info, $map_id ) {
        ...
        foreach ( $json as &$json_obj ) {
            ...
            if ( !array_key_exists( 'tooltipContent', $json_obj ) ) {
                $json_obj['tooltipContent'] = isset( $json_obj['id'] ) ? $json_obj['id'] : '';
            }
    ...
    private function get_json_url_data( $url, $map_id, $type = 'regions' ) {
        ...
        $url_args   = apply_filters( 'igm_json_data_url_args', [] );
        ...
        if (
            ...
            if ( is_wp_error( $request ) ) {
                ...
                if ( json_last_error() !== JSON_ERROR_NONE ) {
                    // if there's an error, keep the previous cached version
                    $data = !empty( $raw_cached ) ? $raw_cached : '';
                    $message .= 'Successfully fetched file, but invalid JSON - returning cached';
                    if ( $debug ) {
                    ...
                    $message .= 'Successfully fetched JSON';
                    $data = $request['body'];
                }
            }

            // add slashes to remove them again to make sure we escape quotes inside a property
            // this was related with an issue of having html inside a property and breaking when deserializing
