...
class ProGeolocateContent {
    ...
    public function __construct( Core $core ) {
        ...
        add_filter( 'igm_model', [ $this, 'add_geolocate_content_settings' ] );
        // prepare map info
        add_filter( 'igm_prepare_meta', [ $this, 'prepare_geo_meta' ], 5 );
        // filter on save post type
        add_filter( 'csf_igm_geolocation_save', [ $this, 'save_control_field' ], 1, 3 );
    ...
    public function init() {
        ...
        if ( isset( $options['geolocate_enable'] ) && $options['geolocate_enable'] && isset( $options['geolocate_types'] ) && is_array( $options['geolocate_types'] ) ) {
            ...
            add_filter( 'igm_model', [ $this, 'add_geolocate_map_option' ], 1 );
        }
    }
    ...
    public function add_geolocate_map_option( $model ) {
        ...
        $model['meta']['map_info']['sections']['general']['fields']['populateFromGeoAdvanced'] = [
            ...
            'title'                        => __( 'Advanced Query Parameters', 'interactive-geo-maps' ),
                              'desc'       => __( 'Use JSON format with WP_Query parameters you want to add to this query. You can use this for example to filter entries from a specific taxonomy.', 'interactive-geo-maps' ),
        ];
    ...
    public function add_geolocate_meta( $post_type ) {
        ...
        if ( class_exists( 'CSF' ) ) {
            ...
            \CSF::createSection(
                $prefix,
                [
                    'title'  => __( 'Options', 'interactive-geo-maps' ),
                    'fields' => [
                        [
                            'id'      => 'enabled',
                            'type'    => 'switcher',
                        ...
                        [
                            'id'         => 'locationHelper',
                            'type'       => 'content',
                            'content'    => __( 'Use the search form below to find out the coordinates or write them directly in the available fields below.', 'interactive-geo-maps' ),
                            'dependency' => [ [ 'enabled', '==', true ], [ 'type', '==', 'specific' ] ],
                        ],
                        [
                            'id'         => 'coordinates',
                            'type'       => 'map',
                            'title'      => __( 'Location Coordinates', 'interactive-geo-maps' ),
                            'dependency' => [ [ 'enabled', '==', true ], [ 'type', '==', 'specific' ] ],
                        ],
                        [
                            'id'         => 'locationHelper',
            ...
            \CSF::createSection(
                $prefix,
                [
                    'title'  => __( 'Content', 'interactive-geo-maps' ),
                    'fields' => [
                        [
                            'id'         => 'contentHelper',
                            'type'       => 'content',
                            'content'    => __( 'The information below will be used in the map when this entry is included.', 'interactive-geo-maps' ),
                            'dependency' => [ [ 'enabled', '==', true, 'all' ] ],
                        ],
                        [
                            'id'         => 'contentHelper2',
                            'type'       => 'content',
                            'content'    => __( 'Since the option to geolocate this entry is disabled, no options display.', 'interactive-geo-maps' ),
            ...
            \CSF::createSection(
                $prefix,
                [
                    'title'  => __( 'Style & Actions', 'interactive-geo-maps' ),
                    'fields' => [
                        [
                            'id'         => 'styleHelper',
                            'type'       => 'content',
